...
import com.example.newrecipeapp.databinding.ActivitySaladsBinding
...
class VeganRecipes : AppCompatActivity() {
    private lateinit var binding: ActivitySaladsBinding
    private lateinit var finalList: List<Hits>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySaladsBinding.inflate(layoutInflater)
    ...
    private fun getAllRecipes() {
        if (Constants.isNetworkAvailable(this)) {
            ...
            val listCall: Call<RecipeResponse> = service.getRecipe(
                Constants.APP_ID, Constants.APP_KEY, "public", q="vegan"
            )
    ...
    private fun setUpDataIntoRecyclerView(recipeList: List<Hits>) {
        ...
        binding.saladsRv.layoutManager = GridLayoutManager(this, 2)
        binding.saladsRv.adapter = itemAdapter
